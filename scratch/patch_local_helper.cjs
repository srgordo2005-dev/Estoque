...
// 1. Fix updateFarmStatus to mark offline machines as offline in cache
...
// 2. Fix the Vnish engine to properly extract chains stock info
const oldVnishEngineRegex = /const extractVnishFullDetails = async \(ip\) => \{[\s\S]*?return \{\n\s*model: model,[\s\S]*?\};\n    \} catch\(e\) \{\n\s*return null;\n    \}\n\};/g;
...
code = code.replace(oldVnishEngineRegex, newVnishEngine);

fs.writeFileSync('local-helper.js', code, 'utf8');

if (fs.existsSync('desktop/local-helper.js')) {
    fs.writeFileSync('desktop/local-helper.js', code, 'utf8');
}
