    ...
    for (const h of all) {
        ...
        if (h.data && typeof h.data === 'string') {
            try {
                ...
                if (parsed.machineSN && parsed.machineSN.toLowerCase().includes('ramon')) {
                    console.log("Hash " + parsed.sn + " has machineSN: " + parsed.machineSN);
                    parsed.machineSN = "";
                    update.data = JSON.stringify(parsed);
                    changed = true;
                }
        ...
        if (changed) {
            console.log("Updating hash ID:", h.id);
